...
def select_master_frame(images_dir: Path, img_paths: list) -> Path:
    """
    Seleciona o melhor frame ("frame mestre") de uma sequência gerada.

    Critérios de seleção:
    1. Score de qualidade via ImageQualityValidator (nitidez, cores, etc)
    2. Preferência por frames centrais da sequência (melhor cobertura 360°)

    O frame mestre é a melhor representação visual do mundo gerado,
    ideal para alimentar o Hunyuan3D-2 para reconstrução 3D.

    Args:
        images_dir: Diretório onde as imagens salvas estão
        img_paths:  Lista de caminhos das imagens na sequência

    Returns:
        Path do frame mestre selecionado
    """
    try:
    ...
    for i, img_path in enumerate(img_paths):
        ...
        if use_validator:
            quality_score = validator.get_quality_score(img_path)
        else:
            quality_score = 50.0  # Default se o validador não está disponível

        # Bônus para frames centrais (melhor cobertura da cena)
        # Quanto mais perto do centro, maior o bônus (máx 10 pontos)
        distance_from_center = abs(i - center_idx)
        max_distance = max(center_idx, 1)
        center_bonus = (1.0 - distance_from_center / max_distance) * 10.0

        total_score = quality_score + center_bonus

        if total_score > best_score:
            best_score = total_score
            best_path = img_path

    print(f"🏆 Frame mestre selecionado: {Path(best_path).name} (score: {best_score:.1f})")
...
def main():
    ...
    # ── World-to-Mesh: Novos argumentos de estilo ──────────────────────
    # Permite gerar mundos a partir de prompts textuais com estilos variados.
    # O StyleManager injeta keywords técnicas automaticamente.
    parser.add_argument("--text_prompt", type=str, default=None,
                        help="[WORLD-TO-MESH] Prompt textual para geração do mundo. "
                             "Ex: 'uma vila medieval com castelo'")
    parser.add_argument("--style", type=str, default="realistic",
                        choices=["minecraft", "rpg", "realistic", "low-poly", "sci-fi"],
                        help="[WORLD-TO-MESH] Estilo artístico. O StyleManager injeta "
                             "keywords técnicas que o modelo entende melhor.")
    parser.add_argument("--save_master_frame", type=str, default=None,
                        help="[WORLD-TO-MESH] Caminho para salvar o frame mestre. "
                             "Usado pela ponte (bridge.py) para enviar ao Hunyuan3D-2.")
    ...
    # ── World-to-Mesh: Aplicar estilo se prompt textual fornecido ──────
    # Quando o usuário passa --text_prompt, o StyleManager enriquece o prompt
    # com keywords técnicas e ajusta parâmetros de inferência por estilo.
    if args.text_prompt is not None:
        try:
            from src.utils.style_manager import StyleManager
            style_mgr = StyleManager()
            enhanced_prompt, style_params = style_mgr.enhance_prompt(
                args.text_prompt, args.style
            )
            print(f"\n🎨 World-to-Mesh Mode:")
            print(f"  - Prompt original: {args.text_prompt}")
            print(f"  - Estilo: {args.style}")
            print(f"  - Prompt enriquecido: {enhanced_prompt}")
            print(f"  - Parâmetros de estilo: {style_params}")

            # Sobrescrever parâmetros de inferência com os do estilo
            # (somente se o usuário não os especificou explicitamente)
            for param_name, param_value in style_params.items():
                if hasattr(args, param_name):
                    setattr(args, param_name, param_value)
                    print(f"    → {param_name} ajustado para {param_value}")
        except ImportError:
            print("⚠️  StyleManager não disponível. Usando prompt sem enriquecimento.")
            enhanced_prompt = args.text_prompt
        except ValueError as e:
            print(f"⚠️  Erro no StyleManager: {e}. Usando prompt sem enriquecimento.")
            enhanced_prompt = args.text_prompt
    ...
    # ── World-to-Mesh: Selecionar e salvar frame mestre ────────────────
    # O frame mestre é a melhor imagem da sequência, selecionada por
    # critérios de qualidade + centralidade. Usado pelo bridge.py como
    # entrada para o Hunyuan3D-2.
    if args.save_master_frame:
        master_frame_path = select_master_frame(images_resized_dir, img_paths)
        # Copiar frame mestre para o destino especificado
        import shutil
        master_dest = Path(args.save_master_frame)
        master_dest.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(str(master_frame_path), str(master_dest))
        print(f"📸 Frame mestre salvo em: {master_dest}")
    if args.save_colmap:
